...
RMSE_posASRUKF100  = zeros(MCRuns,filter.K);
RMSE_velASRUKF100  = zeros(MCRuns,filter.K);
...
%     [5767802.3,-2818455,5876662.9,-4119.834,-572.527,-1294.169]
    ...
    for k = 1:filter.K
        ...
        RMSE_posASRUKF100(iMCruns,k)=sqrt(sum(stateUpd_ASRUKF([1 2 3])-filter.truth_X([1 2 3],k)).^2);
        RMSE_velASRUKF100(iMCruns,k)=sqrt(sum(stateUpd_ASRUKF([4 5 6])-filter.truth_X([4 5 6],k)).^2);
...
x=400;
for a=1:x
error_xASRUKF100(a)=sqrt((error_xASRU(a))^2+(error_yASRU(a))^2+(error_zASRU(a))^2);
error_vASRUKF100(a)=sqrt((error_vxASRU(a))^2+(error_vyASRU(a))^2+(error_vzASRU(a))^2);
end

save('error_xASRUKF100');
save('error_vASRUKF100');

save('RMSE_posASRUKF100');
save('RMSE_velASRUKF100');